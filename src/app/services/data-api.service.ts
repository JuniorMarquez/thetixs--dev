...
export class DataApiService {
 ...
 //info:any={};
  ...
  constructor(public _uw:UserWService,private http: HttpClient, private authService:AuthService) {}
	...
	getAllTixs(){
		const url_api = 'http://localhost:3000/api/product?filter[where][status]=activated';
	...
	getTixById(id:string){
		//console.log(id);
		let indice = id;
		const url_api=`http://localhost:3000/api/product/${indice}`;
	...
	//	let token = this.authService.getToken();
		const url_api='http://localhost:3000/api/product';
		return this.http
		.post<TixInterface>(url_api, tix,{headers: this.headers})
		.pipe(map(data => data));
	}
	...
	getCards(id:string){
		let indice = id;
		const url_api = "http://localhost:3000/api/card?filter[where][userd]=a"+indice;
		return this.http.get(url_api);
	}
	saveCard(card: CardInterface){
		//const token = this.authService.getToken();
		//console.log(token);
		const url_api='http://localhost:3000/api/card';
		return this.http

...
export class DataApiService {
  ...
  constructor(
	...
	getCardById(id:string){
		let indice = id;
		const url_api=`https://www.thetixsapp.com:3000/api/card/${indice}`;
	...
	getUsersPending(){
		const url_api='https://www.thetixsapp.com:3000/api/card?filter[where][status]=pending';
		return (this.cards = this.http.get(url_api));
	}

	getActivePartners(){
		const url_api='https://www.thetixsapp.com:3000/api/card?filter[where][and][0][status]=active&filter[where][and][1][type]=partnerType';
		return (this.cards = this.http.get(url_api));
	}
	getPendingPartners(){
		const url_api='https://www.thetixsapp.com:3000/api/card?filter[where][and][0][status]=pending&filter[where][and][1][type]=partnerType';
		return (this.cards = this.http.get(url_api));
	}

	getActiveAffiliates(){
		const url_api='https://www.thetixsapp.com:3000/api/card?filter[where][and][0][status]=active&filter[where][and][1][type]=affiliateType';
		return (this.cards = this.http.get(url_api));
	}
	getPendingAffiliates(){
		const url_api='https://www.thetixsapp.com:3000/api/card?filter[where][and][0][status]=pending&filter[where][and][1][type]=affiliateType';
		return (this.cards = this.http.get(url_api));
	}
